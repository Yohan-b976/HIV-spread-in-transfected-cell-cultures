...
Rmax = 780  # Valeur de coupure maximale pour le taux de réplication
...
def simulate_infection_with_diffusion(grid_size, N_initial, dx, t, n, coef_de_transfectées,Liste_infection_24h, Liste_infection_48h, Liste_infection_72h, Liste_infection_96h,capacite_replication):
...
dx = 7.2e-6  # Taille des cellules sur la grille
...
for i in range(0,len(Liste_infection_24h),n_repetitions):
    ...
    moyenne_96h = Liste_infection_96h[i:i+n_repetitions]
...
plt.plot(P_infectiosité,Collecte_donnees[-1], label='72h; cell WT avec virus mutant') # si relance la sim il faut mettre un 0 à la place
...
for d in range (len(Collecte_donnees)):
    ...
    for form in range(len(Collecte_donnees[d])):
        transition += [Collecte_donnees[d][form] - Collecte_donnees[-1][form]]
    Mod_Collecte_donnees += [transition]

plt.plot(P_infectiosité,Mod_Collecte_donnees[-1], label='72h; 0.15')
plt.plot(P_infectiosité,Mod_Collecte_donnees[1], label='72h; 0.15')
plt.plot(P_infectiosité,Mod_Collecte_donnees[2], label='72h; 0.2')
plt.plot(P_infectiosité,Mod_Collecte_donnees[3], label='72h; 0.25')
plt.plot(P_infectiosité,Mod_Collecte_donnees[4], label='72h; 0.30')
plt.plot(P_infectiosité,Mod_Collecte_donnees[5], label='72h; 0.35')
