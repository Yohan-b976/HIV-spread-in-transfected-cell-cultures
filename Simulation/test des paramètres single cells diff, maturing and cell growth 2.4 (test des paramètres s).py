...
def infection_probability(N_i,alp):
    lambda_i = alp * N_i * k
...
def simulate_infection_with_diffusion(grid_size, N_initial, dx, t, n, alp, Liste_retour_24, Liste_retour_48, Liste_retour_72, Liste_retour_96):
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                if state[i, j] == NON_INFECTEE:
                    ...
                    p_infection = infection_probability(virus_density[i, j],alp)
...
plt.plot(s,Moyenne_infection_24h, label='24h')
plt.plot(s,Moyenne_infection_48h, label='48h')
plt.plot(s,Moyenne_infection_72h, label='72h')
plt.plot(s,Moyenne_infection_96h, label='96h')


# Ajouter des labels et des légendes
plt.xlabel('valeur de s')
plt.ylabel('Pourcentage de cellule GFP+')
plt.title('Impact de s sur infection ')
plt.legend()
plt.grid(True)
