...
def simulate_infection(grid_size, N_initial):
    ...
    for t in range(timesteps):
        ...
        if t == 24 or t == 48 or t == 72 or t == 95 or t == 125 or t == 160 or t == 200:
            num_infectieuses = np.sum(state == INFECTIEUSE)
            pourcentage_infectieuses = (num_infectieuses / (total_cells-np.sum(state == MORTE))) * 100
            print(f'Timestep {t}: {pourcentage_infectieuses:.2f}% de cellules infectieuses')
