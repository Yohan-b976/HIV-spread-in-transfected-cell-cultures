        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                if state[i, j] == NON_INFECTEE:
                    ...
                    replication_rate[i, j] = min(number_virion_incell[i, j] * alp, Rmax)
...
plt.plot(V,Moyenne_infection_24h, label='24h')
plt.plot(V,Moyenne_infection_48h, label='48h')
plt.plot(V,Moyenne_infection_72h, label='72h')
plt.plot(V,Moyenne_infection_96h, label='96h')


# Ajouter des labels et des légendes
plt.xlabel('valeur de V')
