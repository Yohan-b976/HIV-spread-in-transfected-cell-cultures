...
m = 22  # Nombre d'étapes avant qu'une cellule infectée devienne infectieuse
...
Rmax = 1000  # Valeur de coupure maximale pour le taux de réplication
mu = 0.02  # Probabilité de mort d'une cellule infectieuse
timesteps = 73 # Nombre d'étapes temporelles
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if state[i, j] == NON_INFECTEE:
                    ...
                    p_infection = infection_probability(virus_density[i, j])
                    nombre_virions = int(virus_density[i, j] * p_infection*0.03) + 1
                    if np.random.rand() < p_infection *0.03:
                        new_virus_density[i, j] -= nombre_virions
                        new_number_virion_infection[i, j] += nombre_virions
                        print(number_virion_infection[i, j])
...
dx = 2.5e-6  # Taille des cellules sur la grille
