# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 16:26:26 2024
...
#capacite_replication = 0
# coef_de_transfectées = 0.3 # fraction de cellule transfectées

#Listes conservant les données des simulations
...
def simulate_infection_with_diffusion(grid_size, N_initial, dx, t, n, coef_de_transfectées,Liste_infection_24h, Liste_infection_48h, Liste_infection_72h, Liste_infection_96h):
    """Cette fonction met en place la simulation de l'infection des cellules 
    par un virus du VIH"""
    ...
    for t_step in range(timesteps):
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                if state[i, j] == NON_INFECTEE:
                    ...
                    replication_rate[i, j] = min(capacite_replication * number_virion_infection[i, j] * V, capacite_replication * Rmax)
        ...
        if t_step == 24 or t_step == 48 or t_step == 72 or t_step == 96 or t_step == 121 or t_step == 160:
            ...
            if t_step == 24:
                Liste_infection_24h += [pourcentage_infectieuses]   
            
            if t_step == 48:
                Liste_infection_48h += [pourcentage_infectieuses]
            
            if t_step == 72:
                Liste_infection_72h += [pourcentage_infectieuses]
            
            if t_step == 96:
                Liste_infection_96h += [pourcentage_infectieuses]
    ...
    plt.show()
    return Liste_infection_24h, Liste_infection_48h, Liste_infection_72h, Liste_infection_96h
...
P_infectiosité = np.linspace(0, 0.7, 14)
V_transfectes = [0, 0.15, 0.20, 0.25,0.30, 0.35]
n_repetitions = 10
Collecte_donnees = []


# # Test avec le WT sur mutant avec dif pv réplicatif

# Liste_infection_24h = []
# Liste_infection_48h = []
# Liste_infection_72h = []
# Liste_infection_96h = []
# for capacite_replication in P_infectiosité:
#     for repeat in range(n_repetitions):
#         Liste_infection_24h, Liste_infection_48h, Liste_infection_72h, Liste_infection_96h = simulate_infection_with_diffusion(grid_size, N_initial, dx, t, n, 0,Liste_infection_24h, Liste_infection_48h, Liste_infection_72h, Liste_infection_96h,capacite_replication)


# Moyenne_infection_72h_WT_Mut = []

# for i in range(0,len(Liste_infection_24h),n_repetitions):
#     moyenne_72h = Liste_infection_72h[i:i+n_repetitions]

   
#     Moyenne_infection_72h_WT_Mut.append(sum(moyenne_72h) / len(moyenne_72h))

# Collecte_donnees +=[Moyenne_infection_72h_WT_Mut]

# # Tracé des courbes

# plt.plot(P_infectiosité, Moyenne_infection_72h_WT_Mut, label='72h; WT')

# # Ajouter des labels et des légendes
# plt.xlabel('Pourcentage de virus produit en fonction du WT')
# plt.ylabel('Taux de réplication (GFP+)')
# plt.title('Taux de réplication du virus mutant par rapport au virus WT ')
# plt.legend()
# plt.grid(True)


# Test avec variation de transfectées et de capacité de reproduction du virus
for coef_de_transfectées in V_transfectes:
    ...
    for capacite_replication in P_infectiosité:
        for repeat in range(n_repetitions):
            ...
            Liste_infection_24h, Liste_infection_48h, Liste_infection_72h, Liste_infection_96h = simulate_infection_with_diffusion(grid_size, N_initial, dx, t, n, coef_de_transfectées,Liste_infection_24h, Liste_infection_48h, Liste_infection_72h, Liste_infection_96h)
    
    Moyenne_infection_24h = []
    Moyenne_infection_48h = []
    Moyenne_infection_72h = []
    
    for i in range(0,len(Liste_infection_24h),n_repetitions):
        moyenne_24h = Liste_infection_24h[i:i+n_repetitions]
        moyenne_48h = Liste_infection_48h[i:i+n_repetitions]
        moyenne_72h = Liste_infection_72h[i:i+n_repetitions]
    ...
    plt.plot(P_infectiosité, Moyenne_infection_72h, label=f'72h; {coef_de_transfectées}')
    Collecte_donnees +=[Moyenne_infection_72h]

    
    # Ajouter des labels et des légendes
    plt.xlabel('Fraction du pouvoir réplicatif du virus mutant par rapport au virus WT')
    plt.ylabel('Taux de réplication (GFP+)')
    plt.title('Impact du pouvoir réplicatif du virus mutant par rapport au virus WT')
    plt.legend()
    plt.grid(True)
...
#V_transfectes = [0.15, 0.20, 0.25,0.30, 0.35]

plt.plot(P_infectiosité,Collecte_donnees[0], label='72h; cell WT avec virus mutant') # si relance la sim il faut mettre un 0 à la place
plt.plot(P_infectiosité,Collecte_donnees[1], label='72h; 0.15 de transfectées')
plt.plot(P_infectiosité,Collecte_donnees[2], label='72h; 0.2 de transfectées')
plt.plot(P_infectiosité,Collecte_donnees[3], label='72h; 0.25 de transfectées')
plt.plot(P_infectiosité,Collecte_donnees[4], label='72h; 0.30 de transfectées')
plt.plot(P_infectiosité,Collecte_donnees[5], label='72h; 0.35 de transfectées')
...
Mod_Collecte_donnees = []

for d in range (len(Collecte_donnees)):
    transition = []
    for form in range(len(Collecte_donnees[d])):
        transition += [Collecte_donnees[d][form] - Collecte_donnees[0][form]]
    Mod_Collecte_donnees += [transition]

plt.plot(P_infectiosité,Mod_Collecte_donnees[0], label='72h; cell WT avec virus mutant')
plt.plot(P_infectiosité,Mod_Collecte_donnees[1], label='72h; 0.15 de transfectées')
plt.plot(P_infectiosité,Mod_Collecte_donnees[2], label='72h; 0.2 de transfectées')
plt.plot(P_infectiosité,Mod_Collecte_donnees[3], label='72h; 0.25 de transfectées')
plt.plot(P_infectiosité,Mod_Collecte_donnees[4], label='72h; 0.30 de transfectées')
plt.plot(P_infectiosité,Mod_Collecte_donnees[5], label='72h; 0.35 de transfectées')

# Ajouter des labels et des légendes
plt.xlabel('Fraction du pouvoir réplicatif du virus mutant par rapport au virus WT')
plt.ylabel('Delta du taux de réplication (GFP+)')
plt.title('Différence entre le taux de réplication du virus mutant par rapport au virus WT ')
plt.legend()
plt.grid(True)
